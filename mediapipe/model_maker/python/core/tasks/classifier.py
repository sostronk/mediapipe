...
class Classifier(custom_model.CustomModel):
    ...
    def __init__(self, model_spec: Any, label_names: Sequence[str], shuffle: bool):
    ...
    def _train_model(
        ...
        train_dataset = train_data.gen_tf_dataset(
            batch_size=self._hparams.batch_size, is_training=True, shuffle=self._shuffle, preprocess=preprocessor
        )
        self._hparams.steps_per_epoch = model_util.get_steps_per_epoch(
            steps_per_epoch=self._hparams.steps_per_epoch, batch_size=self._hparams.batch_size, train_data=train_data
        )
        train_dataset = train_dataset.take(count=self._hparams.steps_per_epoch)
        validation_dataset = validation_data.gen_tf_dataset(batch_size=self._hparams.batch_size, is_training=False, preprocess=preprocessor)
        self._model.compile(optimizer=self._optimizer, loss=self._loss_function, metrics=self._metric_functions)

        latest_checkpoint = tf.train.latest_checkpoint(checkpoint_path) if checkpoint_path else None
        if latest_checkpoint:
            print(f"Resuming from {latest_checkpoint}")
            self._model.load_weights(latest_checkpoint)

        best_ckpt_callback = tf.keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(checkpoint_path, "weights" + "epoch{epoch}"), monitor="total_loss", save_best_only=True, verbose=1
        )
    ...
    def evaluate(self, data: dataset.Dataset, batch_size: int = 32) -> Any:
        ...
        """
        ds = data.gen_tf_dataset(batch_size, is_training=False, preprocess=self._preprocess)
