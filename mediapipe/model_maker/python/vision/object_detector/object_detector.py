...
class ObjectDetector(classifier.Classifier):
    ...
    def __init__(
        self, model_spec: ms.ModelSpec, label_names: List[str], hparams: hp.HParams, model_options: model_opt.ObjectDetectorModelOptions
    ) -> None:
        ...
        """
        super().__init__(model_spec=model_spec, label_names=label_names, shuffle=hparams.shuffle)
    ...
    @classmethod
    def create(cls, train_data: ds.Dataset, validation_data: ds.Dataset, options: object_detector_options.ObjectDetectorOptions) -> "ObjectDetector":
        ...
        object_detector = cls(model_spec=spec, label_names=train_data.label_names, hparams=options.hparams, model_options=options.model_options)
        object_detector._create_and_train_model(train_data, validation_data)
        return object_detector

    def _create_and_train_model(self, train_data: ds.Dataset, validation_data: ds.Dataset):
        ...
        self._optimizer = self._create_optimizer(
            model_util.get_steps_per_epoch(steps_per_epoch=self._hparams.steps_per_epoch, batch_size=self._hparams.batch_size, train_data=train_data)
        )
        self._create_model()
        self._train_model(train_data, validation_data, preprocessor=self._preprocessor, checkpoint_path=self._hparams.export_dir)
        self._save_float_ckpt()

    def _create_model(self) -> None:
        """Creates the object detector model."""
        self._model = model_lib.ObjectDetectorModel(model_spec=self._model_spec, model_options=self._model_options, num_classes=self._num_classes)
    ...
    def restore_float_ckpt(self) -> None:
        ...
        """
        self._create_model()
        self._model.load_checkpoint(os.path.join(self._hparams.export_dir, "float_ckpt"), include_last_layer=True)
        self._model.compile()
        self._is_qat = False

    # TODO: Refactor this method to utilize shared training function
    def quantization_aware_training(self, train_data: ds.Dataset, validation_data: ds.Dataset, qat_hparams: hp.QATHParams) -> None:
        ...
        optimizer = tf.keras.optimizers.experimental.SGD(learning_rate=learning_rate_fn, momentum=0.9)
        ...
        train_dataset = train_data.gen_tf_dataset(
            batch_size=qat_hparams.batch_size, is_training=True, shuffle=self._shuffle, preprocess=self._preprocessor
        )
        steps_per_epoch = model_util.get_steps_per_epoch(steps_per_epoch=None, batch_size=qat_hparams.batch_size, train_data=train_data)
        train_dataset = train_dataset.take(count=steps_per_epoch)
        validation_dataset = validation_data.gen_tf_dataset(batch_size=qat_hparams.batch_size, is_training=False, preprocess=self._preprocessor)
        self._model.compile(optimizer=optimizer)
        self._model.fit(x=train_dataset, epochs=qat_hparams.epochs, steps_per_epoch=None, validation_data=validation_dataset)
        self._is_qat = True

    def evaluate(self, dataset: ds.Dataset, batch_size: int = 1) -> Tuple[List[float], Dict[str, float]]:
        """Overrides Classifier.evaluate to calculate COCO metrics."""
        dataset = dataset.gen_tf_dataset(batch_size, is_training=False, preprocess=self._preprocessor)
        losses = self._model.evaluate(dataset)
        coco_eval = coco_evaluator.COCOEvaluator(annotation_file=None, include_mask=False, per_category_metrics=True, max_num_eval_detections=100)
        for batch in dataset:
            x, y = batch
            y_pred = self._model(x, anchor_boxes=y["anchor_boxes"], image_shape=y["image_info"][:, 1, :], training=False)
    ...
    def _create_fixed_anchor(self, anchor_box: List[float]) -> object_detector_writer.FixedAnchor:
        ...
        return object_detector_writer.FixedAnchor(x_center_norm, y_center_norm, width_norm, height_norm)

    def export_model(self, model_name: str = "model.tflite", quantization_config: Optional[quantization.QuantizationConfig] = None):
        ...
        if quantization_config:
            if self._is_qat:
                raise ValueError("Exporting a qat model with a custom quantization_config is not " "supported.")
            else:
                print("Exporting with custom post-training-quantization: ", quantization_config)
        else:
            if self._is_qat:
                print("Exporting a qat int8 model")
                quantization_config = quantization.QuantizationConfig(inference_input_type=tf.uint8, inference_output_type=tf.uint8)
            else:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if quantization_config:
                converter = quantization_config.set_converter_with_quantization(converter, preprocess=self._preprocessor)
        ...
        ssd_anchors_options = object_detector_writer.SsdAnchorsOptions(object_detector_writer.FixedAnchorsSchema(anchors))
    ...
    def _create_optimizer(self, steps_per_epoch: int) -> tf.keras.optimizers.Optimizer:
        ...
        decay_epochs = self._hparams.cosine_decay_epochs if self._hparams.cosine_decay_epochs else self._hparams.epochs
        learning_rate = tf.keras.optimizers.schedules.CosineDecay(init_lr, steps_per_epoch * decay_epochs, self._hparams.cosine_decay_alpha)
        learning_rate = model_util.WarmUp(initial_learning_rate=init_lr, decay_schedule_fn=learning_rate, warmup_steps=warmup_steps)
        return tf.keras.optimizers.experimental.SGD(learning_rate=learning_rate, momentum=0.9)
